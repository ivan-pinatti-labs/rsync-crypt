...
Ported from docker-torrent-box-with-vpn's fix for its own #114: a green
`CodeRabbit` status does not mean a review happened, and the cases below are
the ones that would otherwise merge on the strength of a colour rather than a
review. No containers and no stack state, so these run anywhere:
